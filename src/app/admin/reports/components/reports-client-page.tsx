...
export function ReportsClientPage({ role, isDirectScope }: { role: string; isDirectScope: boolean }) {
  ...
  const handleDownloadPayrollExcel = async () => {
    ...
    try {
      ...
      window.location.href = `/api/reports/payroll?startDate=${startDate}&endDate=${endDate}${isDirectScope ? '&view=direct' : ''}`;
  ...
  const handleDownloadTimesheets = async () => {
    ...
    try {
      const csvStr = await generateTimesheetReport(startDate, endDate, isDirectScope);
  ...
  const handleDownloadLeaves = async () => {
    ...
    try {
      const csvStr = await generateLeaveReport(startDate, endDate, isDirectScope);
      const url = "data:text/csv;charset=utf-8," + encodeURIComponent(csvStr);
      const link = document.createElement("a");
      link.setAttribute("href", url);
